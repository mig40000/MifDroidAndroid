...
public class StringOptimizer {
	...
	private static String traceRegisterValue(String[] array, int currentIndex, String targetRegister) {
		...
		while (searchIndex >= 0 && !array[searchIndex].contains(METHOD_MARKER)) {
			...
			// Case 2: move-result* assignment (pair with invoke)
			else if (isMoveResultLine(line) && line.contains(targetRegister)) {
				String invokeLine = findInvokeForMoveResult(array, searchIndex);
				if (invokeLine != null) {
					String methodInfo = extractMethodInfo(invokeLine);
					if (methodInfo != null) {
						// Try to resolve common toString receivers to concrete strings
						if (methodInfo.equals("toString")) {
							String receiver = extractFirstRegisterFromInvoke(invokeLine);
							if (receiver != null) {
								// Prefer StringBuilder reconstruction when applicable
								if (invokeLine.contains(GenericConstants.STRINGBUILDER_TOSTRING)) {
									return string_builder(array, searchIndex - 1);
								}
								String resolved = traceRegisterValue(array, searchIndex, receiver);
			...
			// Case 4: iget-object (field access)
	...
	private static boolean isMoveResultLine(String line) {
		return line.contains("move-result");
	}

	private static String findInvokeForMoveResult(String[] array, int moveResultIndex) {
		int invokeIndex = moveResultIndex - 1;
		if (invokeIndex >= 0) {
			String candidate = array[invokeIndex];
			if (candidate.contains("invoke-")) {
				return candidate;
			}
		}
		return null;
	...
	private static String extractFirstRegisterFromInvoke(String invokeLine) {
		try {
			int braceStart = invokeLine.indexOf('{');
			int braceEnd = invokeLine.indexOf('}');
			if (braceStart != -1 && braceEnd != -1) {
				String registerStr = invokeLine.substring(braceStart + 1, braceEnd).trim();
				String[] registers = registerStr.split(",");
	...
	public static String analyzeStringManipulationChain(String[] array, int index, String initialStringRegister) {
		...
		try {
			...
			// Skip non-concrete placeholders that are not string-typed
			if (baseString.startsWith("[Field:") && !baseString.contains("Ljava/lang/String;")) {
				return null;
			}
			if (baseString.startsWith("[Method:")) {
				return null;
			}
	...
	public static String analyzeStringComprehensive(String[] array, int index, String register) {
		...
		try {
			...
			if (directValue != null && !directValue.isEmpty()) {
				boolean isMethodPlaceholder = directValue.startsWith("[Method:");
				boolean isNonStringField = directValue.startsWith("[Field:") && !directValue.contains("Ljava/lang/String;");
				if (!isMethodPlaceholder && !isNonStringField) {
